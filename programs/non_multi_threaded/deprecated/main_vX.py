...
KL = np.array([[FOCAL_LEN_L_X, s, WIDTH / 2], [0, FOCAL_LEN_L_Y, HEIGHT / 2], [0, 0, 1]],
              dtype=np.uint16)  # mock intrinsics
KR = np.array([[FOCAL_LEN_R_X, 0, WIDTH / 2], [0, FOCAL_LEN_R_Y, HEIGHT / 2], [0, 0, 1]],
              dtype=np.uint16)  # mock intrinsics
...
def get_translation_parameters(imgL, imgR, log=False):
    ...
    print("keypoints: ", len(keyptsL), "| matches: ", len(matches), "| good matches:", len(good_matches),
    ...
    if log:
        ...
        cv2.imshow("Good matches",
                   cv2.drawMatches(imgL, keyptsL, imgR, keyptsR, good_matches, None, matchColor=(0, 255, 0)))
        cv2.imshow("Better Matches",
                   cv2.drawMatches(imgL, keyptsL, imgR, keyptsR, better_matches, None, matchColor=(255, 0, 255)))
        cv2.waitKey(0)
...
while True:
    imgR = cv2.remap(cv2.cvtColor(PICAM.read(), cv2.COLOR_BGR2BGRA), MAPL1, MAPL2, cv2.INTER_AREA,
                     borderMode=cv2.BORDER_TRANSPARENT)
    imgL = IMAGE_HUB.recv_image()[1]
    SENDER.send_image(RB_IP_MAIN, np.uint8(
        cv2.warpAffine(imgL, TL, (combined_width, HEIGHT)) * mask_realL + cv2.warpAffine(imgR, TR, (
        combined_width, HEIGHT)) * mask_realR))
    IMAGE_HUB.send_reply(b'OK')
