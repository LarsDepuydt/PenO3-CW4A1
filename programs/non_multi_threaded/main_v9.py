...
while True:
    ...
    imgL = IMAGE_HUB.recv_image()[1]
    out = np.uint8(cv2.warpAffine(imgL, MASK_L, (combined_width, HEIGHT)) * mask_realL + cv2.warpAffine(imgR, MASK_R, (combined_width, HEIGHT)) * mask_realR)
    ...
    SENDER.send_image("", np.array([[0, 0, 0]]))
    IMAGE_HUB.send_reply(b'OK')
