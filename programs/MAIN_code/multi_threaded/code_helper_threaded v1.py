...
class Remapping(threading.Thread):
    ...
    def run(self):
        while True:
            self.REMAPPED = cv2.remap(PICAM.read(), MAPL1, MAPL2, cv2.INTER_AREA, borderMode=cv2.BORDER_TRANSPARENT)
    ...
    def run(self):
        while True:
            if not(self.REMAPPED):
                ...
                SENDER.send_image(RB_IP_HELPER, self.REMAPPED)


remap_step = Remapping(REMAPPED)
send_step = Sending(REMAPPED)
