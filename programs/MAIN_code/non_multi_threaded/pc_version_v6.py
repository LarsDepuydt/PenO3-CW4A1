...
KEYPOINT_MASK_X_BOUND = 0.3 # only search for keypoints in this fraction of pixel towards the bound
...
def get_x_combine_assets_transparent_borders(x_t, log=False):
    ...
    print(x_t)
    BLEND_WIDTH = x_t - xLn - xRn
    height, width = imgL.shape[:2]
    imgL_cropped_width = width - 2*xLn
    imgR_cropped_width = width - 2*xRn

    combined_width = imgL_cropped_width + imgR_cropped_width - BLEND_WIDTH
...
TL, TR, combined_width, mask_realL, mask_realR = get_x_combine_assets_transparent_borders(x_t, log=False)
